...
(function($){
  ...
  var SteppedRangeSlider = function(element, options)
  {
    ...
    this.displaySliderValue = function() {
      // get slider values
      var slider_value_one = $obj.slider.slider('values', 0);
      var slider_value_two = $obj.slider.slider('values', 1);
      //set the display text, only if the display text option is available
      if($opts.display_area_class)
      {
        $('.' + $opts.display_area_class).text($obj.getDisplayMask(slider_value_one) + $opts.divider);
      }
      //update hidden fields, if they are available
      if($opts.hidden_fields)
      {
        $("#" + $opts.hidden_fields[0]).val($obj.getRealValue(slider_value_one)).trigger('change');;
        $("#" + $opts.hidden_fields[1]).val($obj.getRealValue(slider_value_two)).trigger('change');; 
      }
    }
