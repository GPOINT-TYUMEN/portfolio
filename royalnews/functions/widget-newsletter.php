...
class feedburner_subscribe_Widget extends WP_Widget {
	...
	function feedburner_subscribe_Widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'feedburner_subscribe-widget', 'RoyalNews: FeedBurner Subscribe', $widget_ops, $control_ops );
	}
...
?>		
			<form action="http://feedburner.google.com/fb/a/mailverify?" method="post" target="_blank">
				<input type="text" name="email" title="<?php _e('Enter Your Email Address','royalnews'); ?>" class="w_def_text" />
				<input type="hidden" name="uri" value="<?php echo $feedburner_subscribe_username; ?>" />
				<button><?php _e('Submit','royalnews'); ?></button>
			</form>
