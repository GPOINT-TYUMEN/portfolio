...
class tags_links_Widget extends WP_Widget {
	...
	function tags_links_Widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'tags_links-widget', 'RoyalNews: Tags', $widget_ops, $control_ops );
	}
		...
		if ($posttags) {
		  foreach($posttags as $tag) {
			  if ($numbers_count < $numbers) {
				  ...
				  $poststags_output .= '
					<a href="'. get_tag_link($tag->term_id) . '">' . $tag->name . $usecount_output . '</a>
				  ';
				  $numbers_count++;
			  }
		  }
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'numbers' => '20', 'usecount' => 'on', 'description' => 'Tags Links' );
?>
