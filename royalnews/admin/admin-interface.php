...
function propanel_siteoptions_add_admin() {
	...
	$tt_page = add_menu_page('Theme Options', 'Theme Options', 'edit_theme_options', 'siteoptions', 'propanel_siteoptions_options_page');
...
function theme_is_pagetemplate_blog($pagetemplate = '') {
	...
	foreach ($result as $value){
		...
		$output_select .=  '<div><br />Page <strong>'.$title.'</strong><br />'.theme_display_blog_categories($wlm_shortname,$page_id).'</div>';
	}
	wp_reset_query();
...
/*-----------------------------------------------------------------------------------*/
function theme_display_blog_categories($wlm_shortname,$page_id) {
...
function propanel_of_ajax_callback() {
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach ($custom_output as $key => $value) 
		{
			...
			delete_option( $wlm_shortname.'_blog_page_id');
			update_option( $wlm_shortname.'_blog_page_id', $blog_items);
		}
...
function propanel_siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "blog_add_cats":
			$output .= theme_is_pagetemplate_blog('template-homepage%');
		break;		
