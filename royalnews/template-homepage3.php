...
?>
          ...
          <div class="category-page">
			...
			<?php
				...
				if (get_option($wlm_shortname."_home3_sticky_posts") == 'true') {
					...
					$posts_per_page = get_option($wlm_shortname."_home3_sticky_count");
					$type = 'post';
				...
				$col1_count = get_option($wlm_shortname."_home3_col1_newscount");
				$col2_count = get_option($wlm_shortname."_home3_col2_newscount");
				...
				if ($wp_query->have_posts()) : while($wp_query->have_posts()) : $wp_query->the_post();
					...
					<?php } ?>
						...
						<?php
							if ( (get_option($wlm_shortname."_home3_col2_newscount") == 0) && ($i > $col1_count) ) { echo ''; } else {
								$custom = get_post_custom($post->ID);
								
								$post_excerpt_output = '';
								if (get_option($wlm_shortname."_home3_post_excerpt") == 'true') {
									$custom_page_description = (isset($custom[$wlm_shortname."_small_news_description"][0])) ? $custom[$wlm_shortname."_small_news_description"][0] : '';
	...
	<?php if (get_option($wlm_shortname."_home3_popular") == 'true') { ?>
    <!-- // maeterials slider start // -->
	<section class="main-content-b">
	  <div class="materials-slider">
		<div class="materials-devider"></div>
		<div class="materials-slider-lbl"><?php echo get_option($wlm_shortname."_home3_popular_title"); ?></div>
		<div id="popular-slider">
		  <?php
			...
			$args = array(
				...
				'post_status' => 'publish',
				'post__not_in'=>get_option('sticky_posts'),
				'posts_per_page' => get_option($wlm_shortname.'_home3_popular_count'),
