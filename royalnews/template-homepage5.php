...
?>  
          ...
          <header class="page-heading">
			...
			<?php
				...
				if (get_option($wlm_shortname."_home5_sticky_posts") == 'true') {
					...
					echo '
						<!-- // photo-slider start // -->
						<div class="photo-slider modify-slider">  
						  <div class="flexslider">
							<ul class="slides">
					';
					
					$posts_per_page = get_option($wlm_shortname."_home5_sticky_count");
					$type = 'post';
					...
					if ($wp_query->have_posts()) : while($wp_query->have_posts()) : $wp_query->the_post();
						...
						if ($i<=$posts_per_page) {
							...
							?>
							  <li>
								  <!-- // -->
								  <div class="photo-slider-i">
									<img src="<?php echo mr_image_resize($image_preview_url,1680,828,'br','',''); ?>" alt="<?php the_title(); ?>" />
          ...
          <div class="category-page">
            ...
            <section class="category-left">
				...
				$col1_count = get_option($wlm_shortname."_home5_col1_newscount");
				$col2_count = get_option($wlm_shortname."_home5_col2_newscount");
				...
				if ($wp_query->have_posts()) : while($wp_query->have_posts()) : $wp_query->the_post();
					...
					//if ($post_number = $col1_count+1) { $post_number = 0; }
					//echo $post_number.' - ';
					...
					if ($format_news == 'standard_thumb' || $format_news == 'mini_thumb') {
						if ( ($format_news == 'standard_thumb') || ($format_news == '') ) {
							$image_preview_output = ($image_preview_url) ? '<a href="'.get_permalink().'" class="article-image"><img src="'.mr_image_resize($image_preview_url,$width,$height,'br','','').'" alt="'.get_the_title().'" /></a>' : '';
							...
							if ( ($template_format == 'slider_post') || ($template_format == 'gallery_post') ) {
								...
								for ($m=0; $m<=count($slider_images)-1; $m++) {
									$image_preview_output .= '
										<li>
										  <span><img src="'.mr_image_resize($slider_images[$m],$width,$height,'br','','').'" alt="'.get_the_title().'" /></span>
										</li>
									';
								}
					...
					<?php } ?>
						...
						<?php
							if ( (get_option($wlm_shortname."_home5_col2_newscount") == 0) && ($i > $col1_count) ) { echo ''; } else {
								...
								$post_excerpt_output = '';
								if (get_option($wlm_shortname."_home5_post_excerpt") == 'true') {
									$custom_page_description = (isset($custom[$wlm_shortname."_small_news_description"][0])) ? $custom[$wlm_shortname."_small_news_description"][0] : '';
             ...
             <?php
				...
				$col1_count = get_option($wlm_shortname."_home5_col1_newscount");
				$col2_count = get_option($wlm_shortname."_home5_col2_newscount");
				$col3_count = get_option($wlm_shortname."_home5_col3_newscount");
	...
	<?php if (get_option($wlm_shortname."_home5_popular") == 'true') { ?>
    <!-- // maeterials slider start // -->
	<section class="main-content-b">
	  <div class="materials-slider">
		<div class="materials-devider"></div>
		<div class="materials-slider-lbl"><?php echo get_option($wlm_shortname."_home5_popular_title"); ?></div>
		<div id="popular-slider">
		  <?php
			...
			$args = array(
				...
				'post_status' => 'publish',
				'post__not_in'=>get_option('sticky_posts'),
				'posts_per_page' => get_option($wlm_shortname.'_home5_popular_count'),
