...
<div class="category-page">
	...
	<section class="category-left">
		...
		<?php
			...
			if (get_option($wlm_shortname."_home2_sticky_posts") == 'true') {
				...
				$posts_per_page = get_option($wlm_shortname."_home2_sticky_count");
				$type = 'post';
				if ($cat_id_inclusion){
					$args = array(
						'post_type' => $type,
					...
					if ($i<=$posts_per_page) {
						...
						?>
						  <div class="category-image">
							<img src="<?php echo mr_image_resize($image_preview_url,1106,566,'br','',''); ?>" alt="<?php the_title(); ?>" />
						  </div>
						  ...
						  <div class="category-page-title"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a></div> 

						  <div class="category-page-text"><?php echo $custom_page_description; ?></div>  
		...
		<?php
			...
			$col1_count = get_option($wlm_shortname."_home2_col1_newscount");
			$col2_count = get_option($wlm_shortname."_home2_col2_newscount");
			$col3_count = (get_option($wlm_shortname."_home2_col3_newscount") > 0) ? get_option($wlm_shortname."_home2_col3_newscount") : 1;
			...
			?>
				<!-- // articles row start // -->
				<div class="articles-row<?php if ($i == ($col1_count+1) ) echo ' nth-row'; ?>">
			<?php } ?>

			<?php
			
				if ($i <= ($col1_count+$col2_count)) { //Main News section 
					$custom = get_post_custom($post->ID);
					
					$post_excerpt_output = '';
					if (get_option($wlm_shortname."_home2_post_excerpt") == 'true') {
						$custom_page_description = (isset($custom[$wlm_shortname."_small_news_description"][0])) ? $custom[$wlm_shortname."_small_news_description"][0] : '';
	...
	<?php if ($i == ($col1_count+$col2_count+1)) { ?>
	<!-- \\ articles row end \\ -->              
	</section>

	<aside class="category-right">
	  <div class="articles-row nth-row">
	<?php } //start third column ?>
	  
	<?php if ($i > ($col1_count+$col2_count)) { //Main News section ?>
		<!-- // news block start // -->
		<?php if ($i == ($col1_count+$col2_count+1)) { //Main News section ?>
			<?php if (get_option($wlm_shortname."_home2_mainnews") == 'true') { ?>
			<!-- // news block start // -->
			<div class="mp-block">
			  <div class="mp-block-lbl"><?php echo get_option($wlm_shortname."_home2_mainnews_title"); ?></div>  
			  <div class="mp-block-content">
				<?php
					$items_chunks = explode(",", get_option($wlm_shortname."_home2_mainnews_items"));
		...
		<?php
			if ( (get_option($wlm_shortname."_home2_col3_newscount") == 0) && ($i > ($col1_count+$col2_count)) ) { echo ''; } else {
				$custom = get_post_custom($post->ID);
				$custom_page_description = (isset($custom[$wlm_shortname."_small_news_description"][0])) ? $custom[$wlm_shortname."_small_news_description"][0] : '';
				$post_excerpt_output = ($custom_page_description) ? '<div class="article-text">'.$custom_page_description.'</div>' : '';
		?>
	...
	<?php if ($i == ($col1_count+$col2_count+$col3_count)) { ?>				
		<div class="load-content"></div>
...
<?php
	//echo '$col1_count+$col2_count+$col3_count'.$col1_count.'+'.$col2_count.'+'.$col3_count; 
	if (
		($i < ($col1_count)) ||
...
$col1_count = get_option($wlm_shortname."_home2_col1_newscount");
$col2_count = get_option($wlm_shortname."_home2_col2_newscount");
$col3_count = get_option($wlm_shortname."_home2_col3_newscount");
$posts_per_page = $col1_count+$col2_count+$col3_count;
...
?>
	...
	<?php if (get_option($wlm_shortname."_home2_popular") == 'true') { ?>
    <!-- // maeterials slider start // -->
	<section class="main-content-b">
	  <div class="materials-slider">
		<div class="materials-devider"></div>
		<div class="materials-slider-lbl"><?php echo get_option($wlm_shortname."_home2_popular_title"); ?></div>
		<div id="popular-slider">
		  <?php
			...
			$args = array(
				...
				'post_status' => 'publish',
				'post__not_in'=>get_option('sticky_posts'),
				'posts_per_page' => get_option($wlm_shortname.'_home2_popular_count'),
